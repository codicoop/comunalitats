...
class User(BaseUser):
    ...
    GENDERS = (("OTHER", "No binari"), ("FEMALE", "Dona"), ("MALE", "Home"))
    ...
    DISCOVERED_US_OPTIONS = (
        ...
        ("PREVIOUS_ACTIVITY", "Per una activitat de la comunalitat"),
        ("OTHER", "Altres"),
    )
    ...
    def clean(self):
        ...
        if self.email != "":
            ...
            if existing_emails.exists():
                errors.update({"email": "Aquest correu ja està en ús."})
        if (
            self.id_number
            and User.objects.filter(id_number__iexact=self.id_number)
            .exclude(id=self.id)
            .exists()
        ):
            errors.update({"id_number": "El DNI ja existeix."})
        if errors:
