...
class GlobalReportYesNoEmptyRow(BaseRow):
    def __init__(
        ...
        self.title = title
        self.comunalitat = self.to_yes_no_empty(comunalitat)
...
class GlobalReportRow(BaseRow):
    ...
    def get_columns(self) -> list:
        return [
            self.title,
            round(self.comunalitat, 1) if self.comunalitat else self.value_if_empty,
